...
- **修复**：`AppTheme` 改为 `DayNight`，创建 `values/colors.xml` + `values-night/colors.xml`，三个主题均添加 `navigationBarColor` + `windowBackground`

#### 修复2：APK启动过渡页面
- **问题**：原 splash 是 Capacitor 默认占位图，视觉效果差
- **修复**：`index.html` 添加内联 CSS splash（TREK logo 脉冲动画 + 加载进度条），删除旧 splash.png，创建 splash.xml

#### 修复3：手机端旅程页面地图不显示高德地图
- **根因**：`MobileMapTimeline.tsx` 和 `JourneyPublicPage.tsx` 硬编码导入 `JourneyMap`（Leaflet），而非 `JourneyMapAuto`（自动切换）
- **修复**：改为导入 `JourneyMapAuto`，尊重用户的地图提供商设置

---

## 2026-06-23 彻底修复移动端APK旅行封面图片不显示(Helmet CORP) v3.0.22-cn.82

### Bug修复

#### 问题
手机端浏览器正常显示旅行封面，但 APK 中封面图片不显示。地点图片正常。

#### 根因（关键发现）
Helmet 默认设置 `Cross-Origin-Resource-Policy: same-origin` 响应头，阻止跨域 no-cors 请求加载资源。

- **封面图片**：通过 `<img src>` 加载 → 跨域 no-cors 请求 → 被 CORP 阻止
- **地点图片**：通过 `fetchPhoto()` → `urlToBase64()` 用 `fetch()` 获取并转为 base64 data URL → data URL 不受 CORP 限制 → 正常显示

Capacitor WebView origin 为 `https://localhost`，请求服务器图片是跨域请求。`<img>` 标签默认发送 no-cors 请求，被 CORP 头阻止。

#### 修复
Helmet 配置添加 `crossOriginResourcePolicy: false`，禁用 `Cross-Origin-Resource-Policy` 响应头。

---

## 2026-06-23 修复移动端APK旅行封面图片不显示(静态文件CORS+CSP) v3.0.22-cn.81

### Bug修复

#### 问题
手机端浏览器和 APK 模式下，旅行封面图片无法显示（显示渐变背景）。手机网页端正常。cn.80 已修复双重路径和按钮可见性，但封面图片仍不显示。

#### 根因（两个因素）

**1. `/uploads/` 静态文件缺少 CORS 响应头**
- `express.static()` 可能未正确继承全局 cors() 中间件的 Access-Control-Allow-Origin 头
- Capacitor WebView (`https://localhost`) 跨域请求服务器静态文件时，缺少 CORS 头导致加载失败
- API 路由（如 `/api/maps/place-photo/:id/bytes`）经过完整 Express 中间件链，CORS 正常 → 地点图标能显示

**2. CSP imgSrc 缺少 `http:`**
- Helmet 的 `imgSrc` 只有 `https:` 没有 `http:`，HTTP 服务器用户的图片被 CSP 阻止

#### 修复
- CSP `imgSrc` 添加 `"http:"`
- `/uploads/avatars`、`/uploads/covers`、`/uploads/journey` 三个静态文件路由添加显式 `cors({ origin: corsOrigin })` 中间件

---

## 2026-06-23 修复旅行封面APK不显示+手机端功能按钮不可见 v3.0.22-cn.80

### Bug修复

#### 问题1：旅行封面在 APK 中不显示
手机 APK 中旅行封面仍然不显示，手机网页端正常显示。

#### 根因
`JourneyDetailPage.tsx` L3134 的设置面板封面预览仍有 `getAssetUrl(`/uploads/${journey.cover_image}`)` 双重路径拼接。后端存储的 `cover_image` 已经是 `/uploads/covers/xxx.jpg` 完整路径，再拼接 `/uploads/` 变成 `/uploads//uploads/covers/xxx.jpg`，导致 404。

之前已修复 JourneyPage.tsx L248/L478 和 JourneyDetailPage.tsx L481，但遗漏了 L3134 和 JourneyPublicPage.tsx L454。

#### 修复
- `JourneyDetailPage.tsx` L3134: `getAssetUrl(`/uploads/${journey.cover_image}`)` → `getAssetUrl(journey.cover_image)`
- `JourneyPublicPage.tsx` L454: 同上

#### 问题2：手机端旅行卡片功能按钮不可见
手机端浏览器和 APK 模式下，"我的旅行"中最近的旅行（SpotlightCard）封面右上方功能按钮（编辑、复制、归档、删除）看不见，但手指点击该区域会触发功能。电脑桌面端鼠标划过会显示。

#### 根因
SpotlightCard 的功能按钮使用 `opacity-0 group-hover:opacity-100`，只在鼠标 hover 时显示。手机端没有 hover 事件，按钮始终 opacity-0 不可见（但 DOM 仍可点击）。

#### 修复
改为 `md:opacity-0 md:group-hover:opacity-100`：
- 手机端（<768px）：无 opacity 类 → 默认 opacity-1（常显）
- 桌面端（≥768px）：opacity-0，group-hover 时 opacity-100（保持原有 hover 显示行为）

---

## 2026-06-23 修复地点图标认证+状态栏深色模式 v3.0.22-cn.79

### Bug修复

#### 问题1：移动端 APK 地点图标不显示
移动端 APK 中大部分地点图标不显示，只有个别地点有图片。

#### 根因
`/api/maps/place-photo/:id/bytes` 路由有 `authenticate` 中间件：
- 网页端：浏览器同源请求自动发送 cookie → 认证通过 → 图片显示
- 移动端：`<img>` 标签跨域请求（`https://localhost` → `https://server`）**不发送 cookie** → 401 → 图片不显示

"个别地点有图片"是因为那些地点的 `image_url` 是 `/uploads/...` 格式（不需要认证），而大部分地点通过 `fetchPhoto()` 获取的 `photoUrl` 是 `/api/maps/place-photo/xxx/bytes`（需要认证）。

#### 修复
移除 `authenticate`，与 `/uploads/avatars` 和 `/uploads/covers` 的安全模型一致。placeId 是不可猜测的 Google/OSM ID，照片是外部公开 POI 图片。

#### 问题2：状态栏深色模式不适配
状态栏颜色不随系统深色模式变化，深色模式下状态栏文字看不清。
...
## 2026-06-23 彻底修复移动端图片URL遗漏 v3.0.22-cn.78

### Bug修复

#### 问题
v3.0.22-cn.75/77 修复后，移动端 APK 封面图片和地点图标仍然不显示。

#### 根因
cn.75 子智能体的修改不完整，有多处遗漏：
1. **DashboardPage**：只修改了 `background-image` 中的 `cover_image`，遗漏了 3 处 `<img src={trip.cover_image}>`
2. **photoService.ts**：服务器返回的 `photoUrl` 是 `/api/maps/place-photo/xxx/bytes` 相对路径，直接用作 img src，移动端解析为 `https://localhost/...` 失败
3. **其他文件**：AdminPage、AccountTab、CollabNotes、CollabChat、JourneyDetailPage 中也有遗漏的 `src={xxx}`

#### 修复
- DashboardPage: 3 处 `<img src={trip.cover_image}>` → `getAssetUrl(trip.cover_image)`
- photoService.ts: photoUrl 和 urlToBase64 参数都包 `getAssetUrl()`
- AdminPage、AccountTab、CollabNotes、CollabChat、JourneyDetailPage: 所有遗漏的 src 都包 `getAssetUrl()`

---

## 2026-06-22 移动端图片URL补充修复 v3.0.22-cn.77

### Bug修复

#### 问题
v3.0.22-cn.75 修复了 18 个文件中的图片URL，但遗漏了关键组件，导致：
- 计划栏、侧边栏中的地点图片（PlaceAvatar）不显示
- 旅行详情中的照片不显示
- 地图上的地点标记图片不显示
- 相册和图片灯箱中的图片不显示

#### 根因
1. `PlaceAvatar` 组件中 `photoSrc` 直接使用 `place.image_url`，没有经过 `getAssetUrl()` 转换
2. 4 个文件中的 `photoUrl()` 工具函数返回相对路径 `/api/photos/...`，移动端会解析为 `https://localhost/api/...` 失败
3. 3 个地图组件中，HTML 模板字符串的 `<img src="...">` 直接使用 `place.image_url` 和 `photoUrl`，没有处理
4. 3 个 PhotoGallery/PhotoLightbox 文件直接使用 `photo.url` 或 `photo.src`

#### 修复
在 11 个文件中应用 `getAssetUrl()`：
- PlaceAvatar、photoUrl() 工具函数（4处）、地图组件（3个）、照片组件（3个）

---

## 2026-06-22 修复移动端状态栏重叠 v3.0.22-cn.76

### Bug修复

#### 问题
APK 中页面内容与手机顶部状态栏图标重叠，且无法点击该区域的按钮。

#### 根因
Capacitor Android 默认让 WebView 延伸到状态栏下方（edge-to-edge 模式）。移动端 Navbar 是 `hidden md:flex`（仅桌面显示），`--nav-h` 在移动端为 `0px`，页面内容从屏幕最顶部开始，直接与状态栏重叠。

#### 修复
1. **`client/src/main.tsx`**：启动时调用 `StatusBar.setOverlaysWebView({ overlay: false })`，让 WebView 不延伸到状态栏下方，内容自动在状态栏下方开始
2. **`client/src/index.css`**：添加 CSS 兜底 `@media (max-width: 767px) { body { padding-top: env(safe-area-inset-top, 0px); } }`，配合 `box-sizing: border-box` 确保布局正确

---

## 2026-06-22 移动端图片资源 URL 修复 v3.0.22-cn.75

### Bug修复

#### 问题
Capacitor 移动端 App 中，所有使用相对路径（如 `/uploads/covers/xxx.jpg`、`/uploads/avatars/xxx.jpg`）的图片 `src` 和 `background-image` URL 都会解析为 `https://localhost/uploads/...`，导致图片无法加载。

#### 根因
移动端 Capacitor WebView 的 origin 是 `https://localhost`，相对路径资源 URL 会解析到 localhost 而非用户配置的服务器地址。虽然 v3.0.22-cn.66 已创建 `getAssetUrl()` 工具函数，但前端 18 个文件中的图片 URL 引用尚未应用该函数。

#### 修复方案
在所有包含图片 `src` 和 `background-image` URL 引用的文件中，导入并应用 `getAssetUrl()` 函数：
- `getAssetUrl()` 在移动端将相对路径转换为完整服务器 URL（如 `/uploads/x.jpg` → `https://server.com/uploads/x.jpg`）
- Web 环境下原样返回相对路径
- 已是绝对 URL（http/https/data/blob）的不处理
- SharedTripPage.tsx 中的复杂条件 URL 逻辑简化为 `getAssetUrl(trip.cover_image)`

### 涉及文件（18个）
#### 页面文件（5个）
- `client/src/pages/DashboardPage.tsx`（3处 trip.cover_image src + 2处 url() + 1处 user.avatar_url）
- `client/src/pages/JourneyPage.tsx`（3处 cover_image src）
- `client/src/pages/JourneyDetailPage.tsx`（2处 cover_image src，合并已有 serverConfig import）
- `client/src/pages/SharedTripPage.tsx`（1处复杂条件 URL 简化 + 1处 place.image_url + 1处 avatar）
- `client/src/pages/JourneyPublicPage.tsx`（1处 cover_image backgroundImage）

#### 组件文件（13个）
- `client/src/components/Layout/Navbar.tsx`（1处 user.avatar_url）
- `client/src/components/Planner/DayPlanSidebar.tsx`（1处 avatar，合并已有 serverConfig import）
- `client/src/components/Planner/PlaceInspector.tsx`（2处 member.avatar_url/avatar）
- `client/src/components/Planner/DayDetailPanel.tsx`（1处 acc.place_image + 1处 p.image_url）
- `client/src/components/Trips/TripMembersModal.tsx`（1处 avatarUrl）
- `client/src/components/Todo/TodoListPanel.tsx`（3处 avatar）
- `client/src/components/Files/FileManager.tsx`（1处 avatarUrl）
- `client/src/components/Collab/CollabChat.tsx`（1处 msg.user_avatar）
- `client/src/components/Collab/CollabPolls.tsx`（1处 voter.avatar_url）
- `client/src/components/Collab/WhatsNextWidget.tsx`（1处 p.avatar）
- `client/src/components/Budget/BudgetPanel.tsx`（2处 avatarUrl + 1处 tm.avatar_url）
- `client/src/components/Notifications/InAppNotificationItem.tsx`（1处 sender_avatar）
- `client/src/components/Packing/PackingListPanel.tsx`（2处 m.avatar）
...
- **根因**：v3.0.22-cn.68 的 CORS 修改引入 bug —— 当 `ALLOWED_ORIGINS` 未设置时，production 模式下只允许 Capacitor 的 `localhost` origin，拒绝了用户正常的网页端 origin（如 `https://your-domain.example.com:9999`）
- **修复**：
...
- **根因**：Capacitor Android 使用 `https://localhost` 作为 origin（因为 `androidScheme: 'https'`），但服务器的 CORS 配置使用 `ALLOWED_ORIGINS` 环境变量白名单。如果用户设置了 `ALLOWED_ORIGINS=https://your-domain.example.com`，那么来自 `https://localhost` 的请求会被 CORS 拒绝，导致 `fetch` 失败
- **修复**：
...
- **错误日志**：
  ...
  Mixed Content: The page at 'https://your-domain.example.com:9999/trips/1' was loaded over HTTPS,
  ...
  ```
- **根因**：Blob URL 文档继承了父页面的 HTTPS origin（`https://your-domain.example.com:9999`）。高德地铁图 JS API 内部用 XMLHttpRequest 请求 `http://webapi.amap.com/subway/data/citylist.json`（HTTP 协议），浏览器阻止了 Mixed Content（HTTPS 页面不允许 HTTP XHR 请求）。请求被阻止后，地铁图数据无法加载，`subway.complete` 事件永远不触发，10 秒后超时
