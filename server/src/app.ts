...
export function createApp(): express.Application {
  ...
  app.use(helmet({
    contentSecurityPolicy: {
      directives: {
        ...
        styleSrc: ["'self'", "'unsafe-inline'", "https://fonts.googleapis.com", "https://fonts.loli.net", "https://webapi.amap.com", "https://*.amap.com"],
  ...
  //
  // CORS: Capacitor mobile WebView (https://localhost) loads these as
  // <img> src cross-origin. The global cors() middleware runs before
  // these static routes, but express.static may short-circuit before
  // response headers are fully applied. We add explicit per-route CORS
  // to ensure Access-Control-Allow-Origin is always present.
  //
  ...
  // trip).
  app.use('/uploads/avatars', cors({ origin: corsOrigin }), express.static(path.join(__dirname, '../uploads/avatars')));
  app.use('/uploads/covers', cors({ origin: corsOrigin }), express.static(path.join(__dirname, '../uploads/covers')));
  app.use('/uploads/journey', cors({ origin: corsOrigin }), express.static(path.join(__dirname, '../uploads/journey')));
