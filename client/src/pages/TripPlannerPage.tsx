...
export default function TripPlannerPage(): React.ReactElement | null {
  ...
  useEffect(() => {
    ...
    for (const mapping of missingMappings) {
      ...
      tripActions.addCategory?.({ name: mapping.name, color: mapping.color, icon: mapping.icon }).catch(err => {
        console.error('[TripPlannerPage] Auto-create category failed:', mapping.name, err)
      })
    }
            ...
            <div className="hidden md:block" style={{ position: 'absolute', left: 10, top: 10, bottom: 10, zIndex: 20 }}>
              ...
              <div style={{
                ...
                <DayPlanSidebar
                  ...
                  onRouteCalculated={(r) => { if (r) { try { const clean = (r.coordinates || []).filter((c): c is [number, number] => Array.isArray(c) && c.length === 2 && typeof c[0] === 'number' && typeof c[1] === 'number'); if (clean.length > 0) { setRoute([clean]); setRouteInfo({ distance: r.distanceText, duration: r.durationText, walkingText: r.walkingText, drivingText: r.drivingText }) } } catch {} } else { setRoute(null); setRouteInfo(null) } }}
                  reservations={reservations}
                  visibleConnectionIds={visibleConnections}
