...
import { getAssetUrl, getApiBaseUrl } from '../utils/serverConfig'
...
function photoUrl(p: { photo_id: number }, size: 'thumbnail' | 'original' = 'thumbnail'): string {
  return getAssetUrl(`/api/photos/${p.photo_id}/${size}`)
}
            ...
            <div className="rounded-none md:rounded-2xl -mx-4 md:mx-0 overflow-hidden relative p-5 md:p-7" style={{ background: pickGradient(current.id), color: 'white' }}>
                {current.cover_image && (
                  <div className="absolute inset-0 z-[1]">
                    <img src={getAssetUrl(current.cover_image)} className="w-full h-full object-cover" alt="" />
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-1.5 mb-1">
                    {group.assets.map((asset: any) => {
                      ...
                      return (
                        ...
                        >
                          <img
                            src={getAssetUrl(`/api/integrations/memories/${provider}/assets/0/${asset.id}/${userId}/thumbnail${selectedAlbumPassphrase ? `?passphrase=${encodeURIComponent(selectedAlbumPassphrase)}` : ''}`)}
                            alt=""
                            className="w-full h-full object-cover"
                            loading="lazy"
                            onError={e => {
                              const img = e.currentTarget
                              const original = getAssetUrl(`/api/integrations/memories/${provider}/assets/0/${asset.id}/${userId}/original${selectedAlbumPassphrase ? `?passphrase=${encodeURIComponent(selectedAlbumPassphrase)}` : ''}`)
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain px-6 py-5 flex flex-col gap-5">
          ...
          <div>
            ...
            >
              {journey.cover_image ? (
                <>
                  <img src={getAssetUrl(journey.cover_image)} className="absolute inset-0 w-full h-full object-cover opacity-50" alt="" />
