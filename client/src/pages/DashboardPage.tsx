  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden rounded-3xl" style={{
        ...
        {trip.cover_image && (
          <>
            <img src={getAssetUrl(trip.cover_image)} className="w-full h-full object-cover transition-transform duration-[1200ms] ease-[cubic-bezier(0.23,1,0.32,1)] group-hover:scale-[1.06]" alt="" />
      ...
      <div className="relative p-6 flex flex-col text-white z-[2]" style={{ minHeight: 340 }}>
        ...
        <div className="flex items-center justify-between mb-5">
          ...
          <div className="flex gap-1.5 md:opacity-0 md:group-hover:opacity-100 transition-opacity" onClick={e => e.stopPropagation()}>
  ...
  return (
    ...
    >
      ...
      <div className="relative h-[120px] overflow-hidden" style={{ background: trip.cover_image ? undefined : tripGradient(trip.id) }}>
        {trip.cover_image && (
          <img src={getAssetUrl(trip.cover_image)} className="absolute inset-0 w-full h-full object-cover transition-transform duration-[800ms] ease-[cubic-bezier(0.23,1,0.32,1)] group-hover:scale-[1.08]" alt="" />
        )}
...
function TripListItem({ trip, onEdit, onCopy, onDelete, onArchive, onClick, t, locale }: Omit<TripCardProps, 'dark'>): React.ReactElement {
  ...
  const coverBg = trip.cover_image
    ? `url(${getAssetUrl(trip.cover_image)}) center/cover no-repeat`
  ...
  return (
    <div onClick={() => onClick(trip)} style={{
      ...
      <div style={{
        ...
        background: trip.cover_image ? `url(${getAssetUrl(trip.cover_image)}) center/cover no-repeat` : tripGradient(trip.id),
        opacity: 0.7,
      }} />
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        <div style={{ maxWidth: 1300, margin: '0 auto', paddingTop: 32, paddingLeft: 20, paddingRight: 20, paddingBottom: 'calc(100px + env(safe-area-inset-bottom, 0px))' }}>
          ...
          <div className="md:hidden flex items-center justify-between mb-5">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                {user?.avatar_url
                  ? <img src={getAssetUrl(user.avatar_url)} className="w-full h-full object-cover" alt="" />
