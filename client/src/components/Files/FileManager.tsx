...
function ImageLightbox({ files, initialIndex, onClose }: ImageLightboxProps) {
  ...
  const scale = useRef(1)          // 总 scale（fitScale * userScale）
  const tx = useRef(0)             // translate x
  const ty = useRef(0)             // translate y
  const fitScale = useRef(1)       // 图片适配视口的初始 scale
  ...
  const onImgLoad = (e: React.SyntheticEvent<HTMLImageElement>) => {
    ...
    if (!iw || !ih) return
    fitScale.current = Math.min(cw * 0.88 / iw, ch * 0.88 / ih, 1)
    scale.current = fitScale.current; tx.current = 0; ty.current = 0
    writeDOM()
  }

  useEffect(() => { resetZoom(false) }, [index])
  useEffect(() => { setImgSrc(''); if (file) getAuthUrl(file.url, 'download').then(setImgSrc) }, [file?.url])
  ...
  const onDblClick = (e: React.MouseEvent) => {
    ...
    const target = fitScale.current * 2.5
  ...
  if (!file) return null
  const hasPrev = index > 0, hasNext = index < files.length - 1
  const navBtn = (side: 'left' | 'right', onClick: () => void, show: boolean) => show ? (
    <button onClick={e => { e.stopPropagation(); onClick() }} style={{ position: 'absolute', top: '50%', [side]: 12, transform: 'translateY(-50%)', zIndex: 10, background: 'rgba(0,0,0,0.5)', border: 'none', borderRadius: '50%', width: 40, height: 40, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'rgba(255,255,255,0.8)' }}>
      {side === 'left' ? <ChevronLeft size={22} /> : <ChevronRight size={22} />}
    </button>
  ) : null

  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.92)', zIndex: 2000, display: 'flex', flexDirection: 'column', paddingBottom: 'var(--bottom-nav-h)', userSelect: 'none', touchAction: 'none' }} onClick={onClose}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', flexShrink: 0 }} onClick={e => e.stopPropagation()}>
        <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.7)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1 }}>
          {file.original_name}<span style={{ marginLeft: 8, color: 'rgba(255,255,255,0.4)' }}>{index + 1} / {files.length}</span>
        </span>
        <div style={{ display: 'flex', gap: 8, flexShrink: 0, alignItems: 'center' }}>
          ...
          <button onClick={() => openFileUrl(file.url, file.original_name).catch(() => {})} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,0.7)', display: 'flex', padding: 4 }}><ExternalLink size={16} /></button>
          <button onClick={() => triggerDownload(file.url, file.original_name)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,0.7)', display: 'flex', padding: 4 }}><Download size={16} /></button>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,0.7)', display: 'flex', padding: 4 }}><X size={18} /></button>
        </div>
      </div>
      <div ref={boxEl} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', minHeight: 0, overflow: 'hidden', touchAction: 'none' }}
        ...
        {imgSrc && <img ref={imgEl} src={imgSrc} alt={file.original_name} onLoad={onImgLoad}
          ...
          onClick={e => e.stopPropagation()} onDoubleClick={onDblClick} draggable={false} />}
        {navBtn('right', goNext, hasNext)}
      </div>
      <div ref={hintEl} style={{ textAlign: 'center', padding: '6px 0', fontSize: 11, color: 'rgba(255,255,255,0.3)', flexShrink: 0 }}>滚轮/双指缩放 · 双击放大 · 键盘 +/- 缩放</div>
      {files.length > 1 && (
        <div style={{ display: 'flex', gap: 4, justifyContent: 'center', padding: '10px 16px', flexShrink: 0, overflowX: 'auto' }} onClick={e => e.stopPropagation()}>
          {files.map((f, i) => <ThumbImg key={f.id} file={f} active={i === index} onClick={() => setIndex(i)} />)}
        </div>
      )}
    </div>
  )
  ...
  return (
    <>
      <div ref={ref} onMouseEnter={onEnter} onMouseLeave={() => setHover(false)}
        ...
        {avatarUrl
          ? <img src={getAssetUrl(avatarUrl)} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
          : name?.[0]?.toUpperCase()
        }
      </div>
