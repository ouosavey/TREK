...
export default React.memo(function PlaceAvatar({ place, size = 32, category }: PlaceAvatarProps) {
  // 移动端：相对路径的图片 URL 需要拼接服务器地址
  const [photoSrc, setPhotoSrc] = useState<string | null>(getAssetUrl(place.image_url) || null)
  ...
  useEffect(() => {
    if (!visible) return
    if (place.image_url) { setPhotoSrc(getAssetUrl(place.image_url)); return }
    if (!placesPhotosEnabled) return
    const photoId = place.google_place_id || place.osm_id
