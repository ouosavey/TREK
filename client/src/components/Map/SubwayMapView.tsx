...
export default function SubwayMapView({ onClose }: SubwayMapViewProps) {
  ...
  useEffect(() => {
    ...
    const html = [
      ...
      '      // 居中：setFitView + setCenter + JS 手动居中 SVG',
      '      function doCenter(){',
      '        try{ si.setFitView(); }catch(e){ console.log("[subway] fitView err:",e); }',
      '        try{ var c=si.getCenter&&si.getCenter(); if(c) si.setCenter(c); }catch(e){}',
      '        // JS 手动居中：获取地铁图容器 div，设置 margin auto',
      '        try{',
      '          var sc=document.getElementById("sc");',
      '          var divs=sc?sc.children:[];',
      '          for(var i=0;i<divs.length;i++){',
      '            var d=divs[i];',
      '            if(d.tagName==="DIV"||d.tagName==="svg"||(d.tagName&&d.tagName.toLowerCase()==="svg")){',
      ...
      '          });',
      '        }',
      '        if(data&&data.line_names){ lineNames=data.line_names; }',
      '        if(data&&data.info&&data.info.lines){ lineNames=data.info.lines; }',
      '      }catch(e){ console.log("[subway] routeComplete parse err:",e); }',
      '      // 备选方案：从 SVG DOM 提取高亮线路名',
      '      if(lineNames.length===0){',
      '        try{',
      '          var texts=document.querySelectorAll("#sc text, #sc tspan");',
      '          var highlighted=[];',
      '          texts.forEach(function(t){',
      '            var el=t.parentElement||t.parentNode;',
      '            var style=el?el.getAttribute("style")||el.getAttribute("class"):"";',
      '            var txt=(t.textContent||"").trim();',
      '            // 线路名通常含"号线"或以"线"结尾',
      '            if(/号线|线$/.test(txt)&&txt.length<10){',
      '              if(highlighted.indexOf(txt)<0) highlighted.push(txt);',
      '            }',
      '          });',
      '          if(highlighted.length>0) lineNames=highlighted;',
