...
export default function SubwayMapView({ onClose }: SubwayMapViewProps) {
  ...
  useEffect(() => {
    ...
    const html = [
      ...
      '#sc{width:100%;height:100%;touch-action:none;position:relative;overflow:hidden}',
      '</style>',
      ...
      '// 缩放范围：扩大到 [0.3, 3.0]，总跨度 2.7，避免双指缩放直接跳到极值',
      ...
      '          if(!sc) return;',
      '          var svg=sc.querySelector("svg")||sc.querySelector("div svg");',
      '          if(!svg) return;',
      '          // 官方文档：setFitView(obj) 参数是站点或线路的 DOM',
      '          try{ si.setFitView(svg); }catch(e){ try{ si.setFitView(); }catch(e2){} }',
      '          // 用 move() 计算偏移量居中（只调用一次！）',
      '          var scRect=sc.getBoundingClientRect();',
      '          var svgRect=svg.getBoundingClientRect();',
      '          var deltaX=(scRect.width-svgRect.width)/2-(svgRect.left-scRect.left);',
      '          var deltaY=(scRect.height-svgRect.height)/2-(svgRect.top-scRect.top);',
      '          if(svgRect.width<scRect.width||svgRect.height<scRect.height){',
      '            si.move(deltaX,deltaY);',
      '            centered=true;',
      '          }',
      '        }catch(e){ console.log("[subway] center err:",e); }',
      '      }',
      '      [500,1500,3000].forEach(function(delay){',
      '        setTimeout(tryCenter,delay);',
      '      });',
      '    });',
      ...
      '              if(op){ opacity=parseFloat(op); break; }',
      '              var st=el.getAttribute("style")||"";',
      '              var m=st.match(/opacity[:\\s]*([\\d.]+)/);',
      '              if(m){ opacity=parseFloat(m[1]); break; }',
      '              el=el.parentElement||el.parentNode;',
      '            }',
      '            if(opacity>0.5){',
      '              if(highlighted.indexOf(txt)<0) highlighted.push(txt);',
      '            }else{',
      ...
      '  var newZoom=Math.max(MIN_ZOOM,Math.min(MAX_ZOOM,curZoom+d));',
      '  // 到达边界时不再调用 scale，避免弹跳',
      '  if(newZoom!==curZoom){ curZoom=newZoom; try{ si.scale(curZoom); }catch(ex){} }',
      '},{passive:false});',
      '',
      '// ── 双指缩放（手机端）──────────────────────────────────────────',
      ...
      '    var rawScale=dist/pinchDist;',
      '    // 灵敏度阻尼：用 pow(rawScale, 0.5) 压缩缩放比例，使双指缩放更平滑',
      '    // rawScale=2.0（两指距离翻倍）→ dampScale=1.41（只放大41%而非100%）',
      '    // rawScale=0.5（两指距离减半）→ dampScale=0.71（只缩小29%而非50%）',
      '    var dampScale=Math.pow(rawScale,0.5);',
      '    var newZoom=Math.max(MIN_ZOOM,Math.min(MAX_ZOOM,pinchZoom*dampScale));',
      '    // 到达边界时不再调用 scale，避免弹跳',
      '    if(newZoom!==curZoom){ curZoom=newZoom; try{ si.scale(curZoom); }catch(ex){} }',
      '  }',
      '},{passive:false});',
