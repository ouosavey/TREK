 ...
 * 高德地铁图 JS API 组件
 *
 * 实现方式：用 iframe 的 srcdoc 属性内嵌 HTML 内容（不走网络请求）。
 *
 * 为什么用 srcdoc 而不是 src：
 * - PWA Service Worker 的 navigateFallback: 'index.html' 会拦截所有导航请求，
 *   包括 iframe 加载的 /subway.html，导致加载的是主应用页面而非 subway.html
 * - 即使用户更新了代码，旧的 SW 缓存可能仍在使用
 * - srcdoc 直接在 iframe 中注入 HTML 字符串，完全不经过网络请求和 SW 拦截
 *
 * 通信：通过 postMessage 与父页面双向通信：
 * - iframe → 父页：subwayReady / subwayComplete / subwayFail / subwayError /
 *                subwayTimeout / subwayLineList / subwayLoading
 * - 父页 → iframe：switchCity / showLine
 */
...
try{si.getLineList(function(l){if(l&&Array.isArray(l))parent.postMessage({type:'subwayLineList',lines:l},'*')}catch(e){}
}catch(e){}
});
...
tid=setTimeout(function(){if(!cr){parent.postMessage({type:'subwayTimeout',msg:'地铁图加载超时，请检查网络或密钥配置'},'*')},10000)
}
window.cbk=function(){
console.log('[subway] cbk invoked');
try{sf=window.subway||window.Subway}catch(e){sf=null}
if(!sf||typeof sf!=='function'){
parent.postMessage({type:'subwayError',msg:'地铁图组件未就绪，请检查密钥是否已开通地铁图服务'},'*');return
}
...
if(e.data.type==='switchCity'){adcode=e.data.adcode;parent.postMessage({type:'subwayLoading'},'*');ci(adcode)}
else if(e.data.type==='showLine'&&si&&e.data.lineName){try{si.showLine(e.data.lineName);var c=si.getSelectedLineCenter&&si.getSelectedLineCenter();if(c)si.setCenter&&si.setCenter(c)}catch(e){}}
});
setTimeout(function(){if(!sf){parent.postMessage({type:'subwayTimeout',msg:'地铁图加载超时，请检查网络或密钥配置'},'*')}},15000);
})();
</script></body></html>`
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: '#ffffff', flexShrink: 0, gap: 6,
      }}>
        {/* 左侧：城市选择器 */}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
          {lineList.length > 0 && (
            <button onClick={() => setShowLinePanel(v => !v)} title="查看线路列表" style={{
              ...
              padding: isMobile ? '5px 8px' : '6px 10px', borderRadius: 6,
              border: '1px solid #d1d5db', background: showLinePanel ? '#f3f4f6' : '#ffffff',
          ...
          {lineList.map((line: any, idx: number) => {
            ...
            return (
              <div key={idx} onClick={() => handleLineClick(line)} style={{
                ...
                whiteSpace: 'nowrap', cursor: 'pointer', transition: 'border-color 0.15s',
              }}
      ...
      <div style={{ position: 'relative', flex: 1, overflow: 'hidden' }}>
        ...
        {loading && !errorMsg && (
          <div style={{
            ...
            color: '#6b7280', fontSize: isMobile ? 12 : 13, gap: 10,
            pointerEvents: 'none',
          }}>
        ...
        {errorMsg && (
          <div style={{
            ...
            display: 'flex', flexDirection: 'column',
            alignItems: 'center', justifyContent: 'center',
            color: '#ef4444', fontSize: isMobile ? 12 : 13,
        ...
        {!loading && !errorMsg && (
          <div style={{
            position: 'absolute',
            right: isMobile ? 8 : 12, bottom: isMobile ? 8 : 12,
            padding: isMobile ? '5px 8px' : '6px 10px', borderRadius: 6,
